...
// Prazo (finalDeadline/originalFinalDeadline) é gravado a partir da DATA
// escolhida no DatePicker — "yyyy-MM-dd", ou meia-noite UTC desse mesmo
// dia quando passa por `new Date(str).toISOString()` na criação (ver
// createVideo em actions.ts). Não é um instante real, é uma data
// "carimbada" em UTC. Passar isso pelo brazilDateKey (pensado pra
// TIMESTAMPS de verdade, tipo updatedAt) subtrai 3h do fuso de Brasília e
// empurra a data pro dia ANTERIOR ao que foi escolhido — é por isso que um
// vídeo entregue no dia certo aparecia como atrasado. A data do PRAZO sai
// direto dos 10 primeiros caracteres da string (já é a data certa, sem
// conversão de fuso); só a data da ENTREGA (updatedAt, um timestamp real)
// precisa do brazilDateKey pra saber em que dia, no fuso de Brasília, a
// entrega aconteceu de fato.
function deadlineDateKey(iso: string) {
...
export function computeOnTimeDelivery(videos: AnalyticsVideo[]) {
  ...
  const onTime = delivered.filter((v) => brazilDateKey(v.updatedAt) <= deadlineDateKey(v.originalFinalDeadline));
  return {
...
export function computeMonthlyOnTime(videos: AnalyticsVideo[], months: string[]) {
  ...
  return months.map((month) => {
    ...
    const onTime = inMonth.filter((v) => brazilDateKey(v.updatedAt) <= deadlineDateKey(v.originalFinalDeadline));
    return {
      month,
...
// ---------------------------------------------------------------------------
// 5. Carga concluída (checklist) — por pessoa e por etapa
// ---------------------------------------------------------------------------
// Diferente de "Utilização da equipe" acima (que é carga PLANEJADA, vinda de
// cutflow_workload_entries via Planejar Semana), isto é carga REALIZADA: a
// soma da carga estipulada de cada item de checklist que alguém de fato
// marcou como concluído no período — respondendo "quem produziu quanto" e
// "em qual etapa o tempo da produtora está indo", sem mexer no sistema de
// capacidade/agendamento que já existe (por decisão de escopo: ver o
// comentário em toggleChecklistItem, actions.ts).
type CompletedChecklistItem = { label: string; estimatedLoadHours: number; completedById: string | null };

export function computeChecklistLoadByPerson(users: AnalyticsUser[], items: CompletedChecklistItem[]) {
  const byUser = users
    .map((u) => {
      const mine = items.filter((i) => i.completedById === u.id);
      return { id: u.id, name: u.name, hours: mine.reduce((acc, i) => acc + i.estimatedLoadHours, 0), count: mine.length };
    })
    .filter((e) => e.count > 0)
  ...
  return {
    byUser,
    totalHours: byUser.reduce((acc, e) => acc + e.hours, 0),
    totalCount: byUser.reduce((acc, e) => acc + e.count, 0),
  };
}

export function computeChecklistLoadByStage(items: CompletedChecklistItem[]) {
  const map = new Map<string, { hours: number; count: number }>();
  for (const item of items) {
    const entry = map.get(item.label) ?? { hours: 0, count: 0 };
    entry.hours += item.estimatedLoadHours;
    entry.count += 1;
    map.set(item.label, entry);
  }
  return Array.from(map.entries())
    .map(([label, { hours, count }]) => ({ label, hours, count }))
    .sort((a, b) => b.hours - a.hours);
}
